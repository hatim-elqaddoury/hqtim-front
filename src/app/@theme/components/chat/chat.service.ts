...
import { botReplies, gifsLinks, imageLinks, pimageLinks } from './bot-replies';
...
export class ChatService {
  ...
  reply(message: string) {
    ...
    if (botReply.type === 'ppic') {
      botReply.reply.files[0].url = pimageLinks[Math.floor(Math.random() * imageLinks.length)];
    }
